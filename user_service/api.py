...
class UserBillingResource(ModelResource):
    ...
    def dehydrate(self, bundle):
        Utils().show_fields(bundle, 'user', 'is_plan_active', 'plan',
                            'date_created', 'date_updated', 'quota_available', 'quota_utilized')
        return bundle
    ...
    def alter_list_data_to_serialize(self, request, data_dict):
        ...
        if isinstance(data_dict, dict):
            if 'meta' in data_dict:
                del(data_dict['meta'])
                data_dict['billing'] = copy.copy(data_dict['objects'])
                del(data_dict['objects'])
        return data_dict
    ...
    def obj_create(self, bundle, request=None, **kwargs):
        try:
            ...
            if email or user is None:
                raise BadRequest("User/Email can't be blank!")
            if email and User.objects.filter(email=email).count() > 0:
                raise BadRequest("Email should be unique!")
            if username and User.objects.filter(username=username).count() > 0:
                raise BadRequest("Username should be unique!")
        except KeyError as missing_key:
            raise BadRequest("Must provide {missing_key} when creating billing for user".format(missing_key=missing_key))
        return super(UserBillingResource, self).obj_create(bundle, **kwargs)
...
class UserProfileResource(ModelResource):
    ...
    def dehydrate(self, bundle, **kwargs):
        """ Works on GET Requests """
        Utils().show_fields(bundle, 'user', 'phone_number', 'title', 'company')
        return bundle
